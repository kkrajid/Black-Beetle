    ...
    useEffect(() => {
        ...
        ws.onmessage = (event) => {
            ...
            if (message.type === 'stockUpdate') {
                const updatedStock = message.data.find(stock => stock.symbol === symbol);
                if (updatedStock) {
                    ...
                    <div className="flex flex-col lg:flex-row justify-center lg:justify-between items-center mt-5">
                        <div className="flex-1 relative flex flex-col lg:flex-row justify-center items-center">
                            <div className="mt-10 lg:mt-0 bg-white rounded-xl items-center justify-center w-fit p-4">
                                <img src={`https://logo.clearbit.com/${stockData.symbol.toLowerCase()}.com`} alt="logo" className="w-[60px] h-[60px] object-contain" />
                            </div>
                        ...
                        <div className="w-full lg:flex-1 flex justify-between items-center mt-10 lg:mt-0">
                            <div className="flex-1 flex flex-col items-center">
                                <span className="font-extrabold text-white text-lg lg:text-2xl">Buy</span>
                            ...
                            <div className="flex-1 flex flex-col items-center">
                                <span className="font-extrabold text-white text-lg lg:text-2xl">{new Date().toISOString().split('T')[0]}</span>
                        ...
                        <div className="flex-1 mt-5 lg:mt-0 flex justify-end items-center flex-col">
                            <div className="w-full lg:w-3/4">
                                <Graph data={[{ price: stockData.price }]} />
                            </div>
                        </div>
                    </div>
                </div>
